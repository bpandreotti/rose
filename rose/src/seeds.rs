...
pub fn tile(tile_type: TileType) -> Seed {
    let right = Point(tile_type.base_to_side_ratio() / 2.0, 0.0);
    let left = -right;
    Seed(vec![
        RobinsonTriangle::from_base(left, right, tile_type, true),
        RobinsonTriangle::from_base(left, right, tile_type, false),
    ])
}
